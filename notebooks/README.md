...
`colab_train.ipynb` runs the full benchmark on a Colab GPU. Neural models use
the GPU; baselines (`multivariate_gaussian`, `bootstrap`, `ccc_garch`) are
included by default so the Drive run folder can be aggregated into a complete
comparison table. You can switch to neural-only runs in the notebook if you
prefer to run baselines locally.
...
The notebook has 9 sections you run top-to-bottom:
...
2. Leave `SKIP_COMPLETED = True` in cell 6. Runs are skipped only when both `artifacts/models/model_final.pt` and successful evaluation metrics exist.
3. Run cell 6 again. New artifacts go to Drive.

## Notes

- **Which branch to clone.** The notebook pulls `develop` by default. To switch (e.g., to `main` after a merge), edit the `BRANCH = 'develop'` line in cell 4. For a fully reproducible run, set `PINNED_COMMIT` to the published commit SHA.
...
- **Device selection.** `tsgen.train` auto-detects CUDA when `torch.cuda.is_available()` is true; the first notebook cell asserts that a GPU is allocated.
